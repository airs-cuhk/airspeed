"""Per-stream status tracker — continuously monitored, no cached state."""
...
class StreamTracker:
    """Tracks per-stream metrics and derives status from live callback data."""
    ...
    def _derive_status(self, name: str, m: StreamMetrics) -> StreamStatus:
        ...
        # Use observed rate for stall detection, fall back to configured period
        ...
        # Stale = no message for 5x the expected interval, minimum 1000 ms
