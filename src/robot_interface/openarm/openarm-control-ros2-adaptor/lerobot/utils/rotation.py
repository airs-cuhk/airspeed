...
class Rotation:
    ...
    def from_euler(cls, seq: str, angles: np.ndarray, degrees: bool = False) -> "Rotation":
        ...
        Args:
            ...
            angles: Euler angles specified in radians (unless degrees=True).
                   Can be array of shape (3,) for single rotation or (N, 3) for N rotations.
            degrees: If True, angles are assumed to be in degrees. Default is False (radians).
        ...
        # Only support common 'xyz' sequence for now (extrinsic)
        # Can be extended to support other sequences if needed
        seq_lower = seq.lower()
        
        if seq_lower == "xyz":
            # Extrinsic XYZ: first rotate around X, then Y, then Z
            # This is equivalent to intrinsic ZYX
            roll, pitch, yaw = angles[0]
            
            # Compute quaternion for each axis rotation
            # Roll around X-axis
            qx_half = roll / 2.0
            q_roll = np.array([np.sin(qx_half), 0, 0, np.cos(qx_half)])
            
            # Pitch around Y-axis
            qy_half = pitch / 2.0
            q_pitch = np.array([0, np.sin(qy_half), 0, np.cos(qy_half)])
            
            # Yaw around Z-axis
            qz_half = yaw / 2.0
            q_yaw = np.array([0, 0, np.sin(qz_half), np.cos(qz_half)])
            
            # Combine rotations: yaw * pitch * roll (right to left composition)
            # First apply roll
            result = cls(q_roll)
            # Then pitch
            result = cls(q_pitch) * result
            # Finally yaw
            result = cls(q_yaw) * result
            
            if single_rotation:
                return result
            else:
                # For multiple rotations, only implemented single for now
                raise NotImplementedError("Multiple rotations not yet implemented")
        else:
            raise NotImplementedError(f"Euler sequence '{seq}' not yet implemented. Only 'xyz' is supported.")
