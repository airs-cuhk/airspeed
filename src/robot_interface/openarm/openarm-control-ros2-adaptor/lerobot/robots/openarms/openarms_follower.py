...
class OpenArmsFollower(Robot):
    ...
    def __init__(self, config: OpenArmsFollowerConfig):
        ...
        # Safety: Track initial positions for total deviation limit
        self._initial_positions: Dict[str, float] = {}
        # Safety: Track last commanded positions for step limit
        self._last_commanded_pos: Dict[str, float] = {}
    ...
    def connect(self, calibrate: bool = True) -> None:
        ...
        # Record initial positions for safety limits
        self._record_initial_positions()

    def _record_initial_positions(self) -> None:
        """Record initial positions for safety deviation tracking (in degrees)."""
        obs = self.get_observation()
        for key, val in obs.items():
            if key.endswith(".pos"):
                # Convert radians to degrees for internal tracking
                self._initial_positions[key] = float(np.rad2deg(val))
                self._last_commanded_pos[key] = float(np.rad2deg(val))
        logger.info(f"Recorded initial positions for safety limits")
    ...
    def get_observation(self) -> Dict[str, Any]:
        ...
        for motor in self.bus_right.motors:
            ...
            # Convert degrees to radians for consistency with policy expectations
            obs_dict[f"right_{motor}.pos"] = np.deg2rad(state.get("position", 0.0))
            obs_dict[f"right_{motor}.vel"] = np.deg2rad(state.get("velocity", 0.0))
        ...
        for motor in self.bus_left.motors:
            ...
            # Convert degrees to radians for consistency with policy expectations
            obs_dict[f"left_{motor}.pos"] = np.deg2rad(state.get("position", 0.0))
            obs_dict[f"left_{motor}.vel"] = np.deg2rad(state.get("velocity", 0.0))
    ...
    def send_action(
        ...
        # NOTE: Actions come in radians from IK solver, but Damiao motors expect degrees
        goal_pos_right = {}
        goal_pos_left = {}
        ...
        if self.config.max_relative_target is not None:
            ...
            # Apply safety limits to right arm (always apply if goal exists)
            if goal_pos_right:
            ...
            elif present_pos_right:
                # If no goal but have present position, ensure no movement
                goal_pos_right = {key: pos for key, pos in present_pos_right.items()}

            # Apply safety limits to left arm (always apply if goal exists)
            if goal_pos_left:
            ...
            elif present_pos_left:
                # If no goal but have present position, ensure no movement
                goal_pos_left = {key: pos for key, pos in present_pos_left.items()}

        # Apply total deviation limit if configured
        if self.config.max_total_deviation is not None:
            for motor_name, position in goal_pos_right.items():
                full_key = f"right_{motor_name}.pos"
                if full_key in self._initial_positions:
                    init_pos = self._initial_positions[full_key]
                    delta = position - init_pos
                    if abs(delta) > self.config.max_total_deviation:
                        clipped_pos = init_pos + np.sign(delta) * self.config.max_total_deviation
                        logger.warning(
                            f"[SAFETY-3] right_{motor_name}: Total deviation limit reached. "
                            f"Clipped from {position:.2f}° to {clipped_pos:.2f}° "
                            f"(init: {init_pos:.2f}°, max_delta: {self.config.max_total_deviation}°)"
                        )
                        goal_pos_right[motor_name] = clipped_pos

            for motor_name, position in goal_pos_left.items():
                full_key = f"left_{motor_name}.pos"
                if full_key in self._initial_positions:
                    init_pos = self._initial_positions[full_key]
                    delta = position - init_pos
                    if abs(delta) > self.config.max_total_deviation:
                        clipped_pos = init_pos + np.sign(delta) * self.config.max_total_deviation
                        logger.warning(
                            f"[SAFETY-3] left_{motor_name}: Total deviation limit reached. "
                            f"Clipped from {position:.2f}° to {clipped_pos:.2f}° "
                            f"(init: {init_pos:.2f}°, max_delta: {self.config.max_total_deviation}°)"
                        )
                        goal_pos_left[motor_name] = clipped_pos
        ...
        # Return the actions that were actually sent (convert back to radians for consistency)
        result = {}
        for motor, val_deg in goal_pos_right.items():
            result[f"right_{motor}.pos"] = np.deg2rad(val_deg)
            # Update last commanded position for next safety check (in degrees)
            self._last_commanded_pos[f"right_{motor}.pos"] = val_deg
        for motor, val_deg in goal_pos_left.items():
            result[f"left_{motor}.pos"] = np.deg2rad(val_deg)
            # Update last commanded position for next safety check (in degrees)
            self._last_commanded_pos[f"left_{motor}.pos"] = val_deg
        return result
    ...
    def _friction_from_velocity(
        ...
        for motor_full_name, velocity in velocity_rad_per_sec.items():
            ...
            friction_torque = amp_tmp * Fc * np.tanh(coef_tmp * k * velocity) + Fv * velocity + Fo
