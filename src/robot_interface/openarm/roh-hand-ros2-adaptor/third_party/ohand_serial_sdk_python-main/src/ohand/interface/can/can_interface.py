...
def CAN_Init(port_name, baudrate):
    ...
    try:
        ...
        channel = os.environ.get("ROH_CAN_CHANNEL", "can2")
        bus = can.interface.Bus(interface="socketcan", channel=channel, bitrate=baudrate)
        print(f"\nCAN bus initialized successfully: channel={channel}, baudrate={baudrate}")
        return bus
